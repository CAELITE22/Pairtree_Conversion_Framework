...
create or replace function  converter_tests.test_use_case_1(
    ...
    select results_eq('select converter.convert(-1,(select converter.get_uom_id_from_abbreviation(-1,''°C'')),(select converter.get_data_category_id_from(-1,''Air Temperature'')),0)',
                        ARRAY[0::real], 'Convert Celsius to Celsius unit Test 1')
    union all
    select results_eq('select converter.convert(-1,(select converter.get_uom_id_from_abbreviation(-1,''°C'')),(select converter.get_data_category_id_from(-1,''Soil Temperature'')),100)',
                        ARRAY[100::real], 'Convert Celsius to Celsius unit Test 2')
    union all
    select results_eq('select converter.convert(-2,(select converter.get_uom_id_from_abbreviation(-1,''°C'')),(select converter.get_data_category_id_from(-1,''Air Temperature'')),0)',
                        ARRAY[32::real], 'Convert Celsius to Fahrenheit unit Test 3')
    union all
    select results_eq('select converter.convert(-2,(select converter.get_uom_id_from_abbreviation(-1,''°C'')),(select converter.get_data_category_id_from(-1,''Soil Temperature'')),37.5)',
                        ARRAY[99.5::real], 'Convert Celsius to Fahrenheit unit Test 4')
    union all
    select results_eq('select converter.convert(-1,(select converter.get_uom_id_from_abbreviation(-1,''m'')),(select converter.get_id_from_data_category(-1,''Height'')),0.8)',
                        ARRAY[800::real], 'Convert Metres to Millimetres unit Test 5')
    union all
    select results_eq('select converter.convert(-2,(select converter.get_uom_id_from_abbreviation(-1,''mm'')),(select converter.get_id_from_data_category(-1,''Height'')),127)',
    ...
    union all
    select throws_ok ('select converter.convert(-4,2,2,null)', 'CF001', (select error_description from converter.response where error_code = 'CF001')::text,
    ...
    union all
    select throws_ok ('select converter.convert(-1,(select converter.get_uom_id_from_abbreviation(-1,''°C'')),(select converter.get_data_category_id_from(-1,''Height'')),2)', 'CF006', (select error_description from converter.response where error_code = 'CF006')::text,
