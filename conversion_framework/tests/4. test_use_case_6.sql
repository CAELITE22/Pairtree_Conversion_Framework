...
create or replace function converter_tests.test_use_case_6_update_default_conversion_set_category_uom(
    ...
    union all
    select ok((select converter.update_target_conversion_set_category(-1, converter.get_conversion_set_id_from_name(-1,'Metric'),converter.get_data_category_id_from_name(-1,'Air Temperature'), converter.get_uom_id_from_abbreviation(-1,'°F'))),'UOM updated OK - Unit Test 1')
    union all
