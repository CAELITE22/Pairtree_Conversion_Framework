...
-- verify functions have been created
    ...
    union all
    select ok((select converter.update_default_conversion_set_category_uom(-1,converter.get_data_category_id_from(-1,'Air Temperature'), converter.get_uom_id_from_abbreviation(-1,'°F'))),'UOM updated OK - Unit Test 1')
    union all
    select results_eq('select uom_id from converter.category_to_conversion_set where conversion_set_id = (select conversion_set_id from converter.user_conversion_set where user_id = -1) and data_category_id = converter.get_data_category_id_from(-1,''Air Temperature'')',
   ...
   -- test error states
    ...
    union all
    select throws_ok ('select converter.update_default_conversion_set_category_uom(2,(select converter.get_data_category_id_from(-1,''Air Temperature'')),-1)', 'CF009', (select error_description from converter.response where error_code = 'CF009')::text,
    'Supplied UOM ID does not exist - Unit Test 7')
    union all
    select throws_ok ('select converter.update_default_conversion_set_category_uom(-5,(select converter.get_data_category_id_from(-1,''Air Temperature'')),(select converter.get_uom_id_from_abbreviation(-1,''°C'')))', 'CF002', (select error_description from converter.response where error_code = 'CF002')::text,
...
create or replace function converter_tests.test_use_case_6_update_default_conversion_set_category_uom(
    ...
    union all
    select ok((select converter.update_target_conversion_set_category(-1, converter.get_conversion_set_id_from_name(-1,'Metric'),converter.get_data_category_id_from(-1,'Air Temperature'), converter.get_uom_id_from_abbreviation(-1,'°F'))),'UOM updated OK - Unit Test 1')
    union all
    select results_eq('select uom_id from converter.category_to_conversion_set where conversion_set_id = (converter.get_conversion_set_id_from_name(-1,''Metric'')) and data_category_id = converter.get_data_category_id_from(-1,''Air Temperature'')',
        'select converter.get_uom_id_from_abbreviation(-1,''°F'')','Confirm update ran successfully - Unit Test 2')
    union all
   -- test error states
    ...
    union all
    select throws_ok ('select converter.update_target_conversion_set_category(2,2,(select converter.get_data_category_id_from(-1,''Air Temperature'')),-1)', 'CF009', (select error_description from converter.response where error_code = 'CF009')::text,
