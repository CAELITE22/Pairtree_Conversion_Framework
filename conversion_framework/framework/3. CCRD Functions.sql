...
CREATE OR REPLACE FUNCTION converter.ccrd(in_user_id int, in_uom text, in_data_category text,
    in_value float)
RETURNS real
language plpgsql
as
...
declare
    ...
    out_uom_id integer  := uom_id from converter.category_to_conversion_set
        where conversion_set_id = fconversion_set_id and data_category_id = fcategory_id;
